...
butil::Status CoprocessorV2::DoExecute(const std::string& key, const std::string& value, bool* has_result_kv,
  ...
  };
  {
    ...
    status = RelExprHelper::TransFromOperandWrapper(0x02, result_operand_ptr, result_serial_schemas_, result_column_indexes_,
                                                    result_record);
    if (!status.ok()) {
      DINGO_LOG(ERROR) << status.error_cstr();
      return status;
...
butil::Status CoprocessorV2::DoRelExprCore(int codec_version, const std::vector<std::any>& original_record,
  ...
  };
  {
    ...
    status = RelExprHelper::TransToOperandWrapper(0x02, original_serial_schemas_, selection_column_indexes_, original_record,
                                                  operand_ptr);
    if (!status.ok()) {
      DINGO_LOG(ERROR) << status.error_cstr();
      return status;
...
butil::Status CoprocessorV2::DoRelExprCoreWrapper(const std::string& key, const std::string& value,
  ...
  return DoRelExprCore(0x02, original_record, result_operand_ptr);
}
